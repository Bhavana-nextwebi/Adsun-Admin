...
import { addCategory, updateCategory, fetchCategoryDataById } from '../../services/categoryService';
...
export const AddCategory= ({ editMode = false, initialData = {}, onSuccess, setSelectedCategory, setEditMode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (editMode && initialData.id) {
        try {
          const data = await fetchCategoryDataById(initialData.id);
          setFormData({
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    e.preventDefault();
    const { valid, errors: validationErrors } = validateCategoryData(formData);
    ...
    if (valid) {
      ...
      try {
        ...
        const action = editMode ? updateCategory : addCategory;
  ...
  return (
    <>
      <ComponentHeader title="Category"/>

      <div className="row">
        <div className="col-xxl-12">
          <div className="card mt-xxl-n5">
            <div className="card-header">
              <h5 className="mb-sm-1 mt-sm-1">{editMode ? 'Update Category' : 'Add Category'}</h5>
            </div>

            <div className="card-body p-4">
              <form onSubmit={handleSubmit} method="POST">
                <div className="row">
                  {['categoryName'].map((field, index) => (
